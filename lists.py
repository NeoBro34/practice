...
letters. append("c")  # eng so'ngdan qo'shadi xuddi (push())
print(f"the append letters: (letters)")

letters. insert(0, "z")  # oldidan qo'shadi (shift())
print(f"the insert letters: (letters)")

size = len(letters) - 1
resulti = letters.pop(size)  # eng so'ngisini olib tashlaydi (pop())
print(f"the pop resulti: {resulti} and letters: {letters}")
...
for element in enumerate(animals):  # list
